...
- **Điều khiển đọc code nằm trên thanh công cụ của từng cửa sổ code** (`mountViewControls`, bên trái `.editor-bar`; là tuỳ chọn toàn site nên mọi thanh tự vẽ lại qua sự kiện `co1005-view`). Trang có code nhưng không có cửa sổ code (supplementary) thì `A− / A+` và theme code nằm ở topbar, không có Wrap. **Save…** dùng `showSaveFilePicker` để chọn thư mục + tên file (Chrome/Edge); Safari/Firefox không có API này nên rơi về tải xuống thư mục Downloads. Chi tiết: `A− / A+` đổi `--code-scale` (0.85–2, key `co1005-code-scale`) — mọi bề mặt code nhân font-size với biến này (editor, gutter, stdin, output, code-card, snippet quiz, lời giải); `↩ Wrap` bật/tắt soft-wrap (`data-code-wrap="off"` trên `<html>`, key `co1005-wrap`, mặc định bật).
